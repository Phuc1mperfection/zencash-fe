...
export function GoalSpendingList({ goals = [], onEdit, onDelete }: Props) {
  // If goals is not an array (undefined, null, or not actually an array), return empty grid
  if (!Array.isArray(goals) || goals.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">
          No spending goals available for this period.
        </p>
      </div>
    );
  }
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {goals.map((goal) => {
        const percentage = Math.round(
          (goal.spentAmount / goal.goalAmount) * 100
        );
